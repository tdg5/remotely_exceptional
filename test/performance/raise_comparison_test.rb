...
class RaiseComparisonTest < RemotelyExceptional::TestCase
  ...
  context "relative to raise given a max deviation of 0.25" do
    should "perform within 2.3x raise when continuing" do
      ...
      opts = {
        :factor => 2.3,
        :maximum_sample_deviation => 0.25,
      }

      assert_relative_performance(standard, candidate, opts)
    end

    should "perform within 2.3x raise when retrying" do
